...
def list_runs(limit=50):
    sql = (
        "SELECT run_id, MIN(ts) AS start_ts, MAX(ts) AS end_ts, "
    ...
    with _conn() as conn:
        return [dict(r) for r in conn.execute(sql, (int(limit),))]

def delete_before(date_str):
    """Delete events strictly older than local date 'YYYY-MM-DD'. Returns count."""
    with _conn() as conn:
        cur = conn.execute(
            "DELETE FROM events WHERE date(ts,'unixepoch','localtime') < ?",
            (date_str,),
        )
        return cur.rowcount
